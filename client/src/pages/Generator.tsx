...
export default function Generator() {
  ...
  useEffect(() => {
    ...
    loadInvoices();
    api.trash.count().then(r => setTrashCount(r.count)).catch(() => {});
  }, [org]);
  ...
  // Set quote mode from navigation - number is generated by loadInvoices once list is known
  useEffect(() => {
    if (location.state?.mode === 'quote' && org) {
      setForm(f => ({ ...f, type: 'quote', status: 'draft' }));
    }
  }, [org]);
  ...
  const handleSendEmail = async () => {
    ...
    setSending(true);
    const docLabel = savedInvoice.type === 'receipt' ? 'Receipt' : savedInvoice.type === 'quote' ? 'Quote' : 'Invoice';
    try {
      ...
      <header className="bg-white/90 glass border-b border-slate-100 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 flex items-center justify-between gap-2" style={{ height: '68px' }}>
          ...
          <div className="flex items-center gap-1">
            ...
            {!isDemo && (
              <button onClick={() => navigate('/trash')} className="hidden md:flex relative p-2 rounded-xl text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-all" title="Trash">
                <Trash2 className="w-4 h-4" />
                {trashCount > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 w-3.5 h-3.5 rounded-full bg-red-500 text-white text-[8px] flex items-center justify-center font-black leading-none">
        ...
        <div className="fixed inset-0 z-50 md:hidden" onClick={() => setShowMobileMenu(false)}>
          ...
          >
            ...
            <nav className="flex-1 overflow-y-auto py-2">
              ...
              <button
                onClick={() => { navigate('/trash'); setShowMobileMenu(false); }}
                className="w-full flex items-center gap-3 px-5 py-3.5 text-sm font-bold text-slate-700 hover:bg-slate-50 transition-colors"
              >
                <div className="relative w-9 h-9 bg-slate-100 rounded-xl flex items-center justify-center shrink-0">
                  <Trash2 className="w-4 h-4 text-slate-500" />
                  {trashCount > 0 && (
                    <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-red-500 text-white text-[9px] flex items-center justify-center font-black leading-none">
                      {trashCount > 9 ? '9+' : trashCount}
                    </span>
                  )}
                </div>
                ...
                {trashCount > 0 && (
                  <span className="w-5 h-5 rounded-full bg-red-100 text-red-600 text-[10px] flex items-center justify-center font-bold shrink-0">
                    {trashCount > 9 ? '9+' : trashCount}
                  </span>
                )}
              </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => !deleting && setDeleteTarget(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xs p-6 text-center" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-sm text-slate-500 mb-5">
              <span className="font-semibold text-slate-700">{deleteTarget.number}</span> will be moved to Trash. You can restore it or delete it permanently from there.
            </p>
            <div className="flex gap-2">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 sm:p-6 hover-lift animate-fade-up delay-75">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className={LABEL}>{form.type === 'invoice' ? 'Invoice' : form.type === 'receipt' ? 'Receipt' : 'Quote'} Number</label>
      ...
      {showSend && savedInvoice && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md animate-bounce-in overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <h2 className="text-base font-black text-slate-800">Send {savedInvoice.type === 'receipt' ? 'Receipt' : savedInvoice.type === 'quote' ? 'Quote' : 'Invoice'}</h2>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex items-center gap-2">
                <div className={cn('flex items-center gap-1.5 flex-1 px-2.5 py-1.5 rounded-lg text-[11px] font-semibold', sendEmail ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-50 text-slate-400')}>
                  <div className={cn('w-1.5 h-1.5 rounded-full', sendEmail ? 'bg-emerald-500' : 'bg-slate-300')} />
                  {sendEmail ? 'Email ready' : 'No email'}
                </div>
                <div className={cn('flex items-center gap-1.5 flex-1 px-2.5 py-1.5 rounded-lg text-[11px] font-semibold', sendPhone ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-50 text-slate-400')}>
                  <div className={cn('w-1.5 h-1.5 rounded-full', sendPhone ? 'bg-emerald-500' : 'bg-slate-300')} />
                  {sendPhone ? 'WhatsApp ready' : 'No phone'}
                </div>
                <div className={cn('flex items-center gap-1.5 flex-1 px-2.5 py-1.5 rounded-lg text-[11px] font-semibold', sendPhone ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-50 text-slate-400')}>
                  <div className={cn('w-1.5 h-1.5 rounded-full', sendPhone ? 'bg-emerald-500' : 'bg-slate-300')} />
                  {sendPhone ? 'SMS ready' : 'No phone'}
                </div>
              </div>
