...
import { isValidEmailSyntax, domainAcceptsMail, getEmailDomain } from '../utils/emailValidation';
import { isDisposableEmailDomain } from '../utils/disposableEmail';
...
router.post('/', signupIpLimiter, signupSubnetLimiter, async (req: Request, res: Response) => {
  ...
  db.prepare(`
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
  `).run(id, name, trimmedEmail, phone, address, city, state, zip, country || 'US', website,
    ...
    fingerprint_hash || null, verificationStatus, risk.score, risk.action);
